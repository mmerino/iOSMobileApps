...
-(MKAnnotationView *)mapView:(MKMapView *)_mapView viewForAnnotation:(id<MKAnnotation>)annotation{
    ...
    if (annotationView == nil) {
        annotationView = [[StopsAnnotationView alloc] initWithAnnotation:annotation reuseIdentifier:@"AnnoId"];
...
- (IBAction)changeColorAction:(id)sender {
    ...
    if (colorThing == 0) {
        colorString = @"red";
        colorThing = 1;
    } else if (colorThing == 1) {
        colorString = @"blue";
        colorThing = 2;
    } else if (colorThing == 2) {
        colorString = @"green";
        colorThing = 3;
    } else if (colorThing == 3) {
        colorString = @"yellow";
        colorThing = 0;
    }
    
    NSNotificationCenter *notificationCenter = [NSNotificationCenter defaultCenter];
    [notificationCenter postNotificationName:@"COLOR" object:colorString];
